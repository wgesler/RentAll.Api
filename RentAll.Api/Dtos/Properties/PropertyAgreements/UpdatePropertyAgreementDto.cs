...
public class UpdatePropertyAgreementDto
{
    ...
    public (bool IsValid, string? ErrorMessage) IsValid()
    {
        ...
        if (!Enum.IsDefined(typeof(OwnerPaymentType), OwnerPaymentTypeId))
            return (false, $"Invalid OwnerPaymentType value: {OwnerPaymentTypeId}");
    ...
    public PropertyAgreement ToModel(PropertyAgreement existing, Guid organizationId)
    {
        return new PropertyAgreement
        {
            ...
            IsMonthly = IsMonthly ?? existing.IsMonthly,
            HourlyLaborCost = HourlyLaborCost ?? existing.HourlyLaborCost,
            BankName = BankName,
