...
public class PropertyCreateDto
{
	...
	public Property ToModel(PropertyCreateDto p, Guid currentUser)
	{
		return new Property
		{
			...
			DepartureFee = p.DepartureFee,
			MaidServiceFee = p.MaidServiceFee,
			PetFee = p.PetFee,
			Bedrooms = p.Bedrooms,
			Bathrooms = p.Bathrooms,
			Accomodates = p.Accomodates,
			SquareFeet = p.SquareFeet,
			Bedroom1 = (BedSizeType)p.BedroomId1,
			Bedroom2 = (BedSizeType)p.BedroomId2,
			Bedroom3 = (BedSizeType)p.BedroomId3,
			Bedroom4 = (BedSizeType)p.BedroomId4,
			Address1 = p.Address1,
			Address2 = p.Address2,
			Suite = p.Suite,
			City = p.City,
			...
			KeypadAccess = p.KeypadAccess,
			MasterKeyCode = p.MasterKeyCode,
			TenantKeyCode = p.TenantKeyCode,
			Kitchen = p.Kitchen,
			Oven = p.Oven,
