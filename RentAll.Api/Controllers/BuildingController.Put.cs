...
using RentAll.Api.Dtos.Organizations.Buildings;
...
namespace RentAll.Api.Controllers
{
    public partial class BuildingController
    {
        ...
        public async Task<IActionResult> Update([FromBody] BuildingUpdateDto dto)
        {
            ...
            try
            {
                var existingBuilding = await _officeRepository.GetBuildingByIdAsync(dto.BuildingId, CurrentOrganizationId);
                ...
                {
                    if (await _officeRepository.ExistsBuildingByCodeAsync(dto.BuildingCode, CurrentOrganizationId, dto.OfficeId))
                        return Conflict("Building Code already exists");
                }

                var building = dto.ToModel();
                var updatedBuilding = await _officeRepository.UpdateBuildingByIdAsync(building);
