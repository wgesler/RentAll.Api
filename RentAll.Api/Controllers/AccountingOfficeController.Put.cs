	...
	public partial class AccountingOfficeController
	{
		...
		public async Task<IActionResult> Update([FromBody] UpdateAccountingOfficeDto dto)
		{
			...
			try
			{
				...
				var existingAccountingOffice = await _accountingOfficeRepository.GetByIdAsync(CurrentOrganizationId, dto.OfficeId);
				...
				var updatedAccountingOffice = await _accountingOfficeRepository.UpdateAsync(accountingOffice);
				...
				{
					response.FileDetails = await _fileService.GetFileDetailsAsync(updatedAccountingOffice.LogoPath);
				}
				return Ok(response);
			}
			catch (Exception ex)
