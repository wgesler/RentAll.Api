	...
	public partial class AccountingOfficeController
	{
		...
		public async Task<IActionResult> Create([FromBody] CreateAccountingOfficeDto dto)
		{
			...
			try
			{
				...
				var existing = await _accountingOfficeRepository.GetByIdAsync(dto.OrganizationId, dto.OfficeId);
				if (existing != null)
				...
				var createdAccountingOffice = await _accountingOfficeRepository.CreateAsync(accountingOffice);
				...
				if (!string.IsNullOrWhiteSpace(createdAccountingOffice.LogoPath))
					response.FileDetails = await _fileService.GetFileDetailsAsync(createdAccountingOffice.LogoPath);
