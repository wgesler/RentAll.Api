...
public interface IAccountingManager
{
    ...
    Task<CloseAccountingPeriodResult> CloseJournalEntriesThroughClosedPeriodAsync(Guid organizationId, int officeId, int softClosedMonth, int softClosedYear, int startMonth, int startYear, Guid currentUser);
    ...
    Task ApplyReconcileClearPostingAsync(IReadOnlyList<ReconcileJournalEntryLineMark> lines, Guid organizationId, Guid currentUser);
    Task ApplyReconcileCompleteSoftCloseAsync(IReadOnlyList<ReconcileJournalEntryLineMark> lines, Guid organizationId, Guid currentUser);
    ...
    Task<JournalEntrySyncResult> SyncPaymentJournalEntriesAsync(Guid organizationId, string officeIds, Guid currentUser, IProgress<JournalEntrySyncProgress>? progress = null, bool syncDocumentLinksAtEnd = true);
