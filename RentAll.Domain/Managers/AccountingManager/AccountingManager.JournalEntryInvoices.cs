    ...
    private async Task<AccountingJournalEntryResult> CreateJournalEntryFromPaymentWithResultAsync(Invoice invoice, LedgerLine paymentLedgerLine, Guid currentUser)
    {
        ...
        try
        {
            ...
            if (splitAllocations.Count > 0)
            {
                ...
                await CreateAutoGeneratedJournalEntryAsync(paymentJournalEntry);
                ...
                foreach (var splitAllocation in splitAllocations)
                {
                    ...
                    {
                        var prePaymentResult = await CreateJournalEntriesFromPrePaymentWithResultAsync(splitInvoice, splitPaymentLedgerLine, chartOfAccounts, accountingOffice, currentUser);
                        if (prePaymentResult.HasWarning)
                            return AccountingJournalEntryResult.WarningResult(prePaymentResult.Warning!, paymentJournalEntry);
                    }
                }
                ...
                if (unappliedAmount > 0)
                {
                    ...
                    var unappliedPrePaymentResult = await CreatePrePaymentReceivedJournalEntryWithResultAsync(invoice, unappliedLedgerLine, chartOfAccounts, accountingOffice, currentUser);
                    if (unappliedPrePaymentResult.HasWarning)
                        return AccountingJournalEntryResult.WarningResult(unappliedPrePaymentResult.Warning!, paymentJournalEntry);
                }
                ...
                return AccountingJournalEntryResult.Success(paymentJournalEntry);
            }
