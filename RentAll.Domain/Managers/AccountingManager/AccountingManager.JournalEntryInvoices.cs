...
public partial class AccountingManager
{
    ...
    {
        var receivedEntry = await CreatePrePaymentReceivedJournalEntryAsync(invoice, paymentLedgerLine, chartOfAccounts, accountingOffice, currentUser);
        if (receivedEntry == null)
            return null;

        var applyEntry = await CreatePrePaymentApplyJournalEntryAsync(invoice, paymentLedgerLine, chartOfAccounts, accountingOffice, currentUser);
        if (applyEntry == null)
            return null;
    ...
    private async Task LogInvoiceAccountingErrorAsync(Invoice invoice, string message, Guid currentUser)
    {
