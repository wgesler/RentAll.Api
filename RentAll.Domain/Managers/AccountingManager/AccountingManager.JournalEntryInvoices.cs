...
public partial class AccountingManager
{
        ...
        Guid currentUser)
    {
        ...
        // Claim-from-working prevents later steps from updating JEs already pruned/updated (stale id).
        // When empty (post-clear), every upsert is create-only and orphan delete is a no-op.
        ...
        try
        {
            ...
            if (splitAllocations.Count > 0)
            {
                ...
                var updatedPayment = await UpsertClaimedAutoGeneratedJournalEntryAsync(
                    paymentJournalEntry,
                    workingEntries,
                    entry => IsStandardInvoicePaymentJournalEntry(entry, prePaymentAccountId),
                    retainedEntryIds,
                    currentUser,
        ...
        Guid currentUser)
    {
        ...
        try
        {
            ...
            var existingOwnerActual = ClaimFirstMatchingEntry(workingEntries, IsInvoiceOwnerActualPaymentJournalEntry);
    ...
    private async Task<(decimal FirstPeriodCharge, decimal SecondPeriodCharge)?> GetCrossPeriodChargesForPaymentSplitAsync(Invoice invoice, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, DateOnly firstAccountingPeriod, DateOnly secondAccountingPeriod)
    {
        ...
        // Second-period charge JEs use a hashed SourceId (same as refresh); include them so payment
        // split caps use posted Dec/Jan AR instead of falling back to day-ratio of TotalAmount.
        if (TryCreateCrossPeriodInvoiceSlices(invoice, out _, out var secondPeriodInvoice))
        {
            ...
            if (secondPeriodSourceId != invoice.InvoiceId)
            {
                var secondPeriodEntries = (await _journalEntryRepository.GetJournalEntriesAsync(new JournalEntryGetCriteria
                {
                    OrganizationId = invoice.OrganizationId,
                    OfficeIds = invoice.OfficeId.ToString(),
                    SourceTypeId = (int)SourceType.Invoice,
                    SourceId = secondPeriodSourceId,
                    IncludeVoided = false,
                    IncludeUnposted = true
                })).ToList();
                sourceJournalEntries.AddRange(secondPeriodEntries);
            }
        }
