...
public partial class AccountingManager
{
        ...
        try
        {
            ...
            if (splitAllocations.Count > 0)
            {
                ...
                foreach (var splitAllocation in splitAllocations)
                {
                    ...
                    {
                        var ownerPaymentResult = await CreateOwnerSharePaymentJournalEntryWithResultAsync(splitInvoice, splitPaymentLedgerLine, chartOfAccounts, accountingOffice, currentUser, paymentJournalEntry);
                        if (ownerPaymentResult.HasWarning)
            ...
            {
                var ownerPaymentResult = await CreateOwnerSharePaymentJournalEntryWithResultAsync(invoice, paymentLedgerLine, chartOfAccounts, accountingOffice, currentUser, journalEntry);
                if (ownerPaymentResult.HasWarning)
                    return ownerPaymentResult;
            }
    ...
    private async Task<AccountingJournalEntryResult> CreatePrePaymentApplyJournalEntryWithResultAsync(Invoice invoice, LedgerLine paymentLedgerLine, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, Guid currentUser)
    {
        try
        {
            ...
            var created = await CreateAutoGeneratedJournalEntryAsync(journalEntry);
            var ownerPaymentResult = await CreateOwnerSharePaymentJournalEntryWithResultAsync(invoice, paymentLedgerLine, chartOfAccounts, accountingOffice, currentUser, created);
            if (ownerPaymentResult.HasWarning)
                return ownerPaymentResult;
            return AccountingJournalEntryResult.Success(created);
        }
