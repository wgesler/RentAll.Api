...
public partial class AccountingManager
{
    ...
    private async Task<AccountingJournalEntryResult> UpsertInvoicePaymentSideEffectsAsync(Invoice invoice, LedgerLine paymentLedgerLine, IReadOnlyList<JournalEntry> existingPaymentEntries, Guid currentUser, bool createMainCashJournalEntry)
    {
        ...
        try
        {
            ...
            if (splitAllocations.Count > 0)
            {
                ...
                JournalEntry? updatedPayment = null;
                if (createMainCashJournalEntry)
                {
                    ...
                    updatedPayment = await UpsertClaimedAutoGeneratedJournalEntryAsync(
                        paymentJournalEntry,
                        workingEntries,
                ...
                foreach (var splitAllocation in splitAllocations)
                {
                    ...
                    if (IsInvoicePrePayment(splitInvoice, splitPaymentLedgerLine))
                    {
                        var receivedResult = await UpsertPrePaymentReceivedJournalEntryAsync(
                            ...
                            retainedEntryIds,
                            currentUser,
                            targetAccountingPeriod: splitInvoice.AccountingPeriod);
            ...
            if (createMainCashJournalEntry)
            {
                ...
                updatedJournalEntry = await UpsertClaimedAutoGeneratedJournalEntryAsync(
                    journalEntry,
                    workingEntries,
                    entry => IsStandardInvoicePaymentJournalEntry(entry),
                    retainedEntryIds,
    ...
    private async Task<AccountingJournalEntryResult> UpsertPrePaymentReceivedJournalEntryAsync(Invoice invoice, LedgerLine paymentLedgerLine, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, List<JournalEntry> workingEntries, int prePaymentAccountId, ISet<Guid> retainedEntryIds, Guid currentUser, DateOnly? targetAccountingPeriod = null)
    {
        try
        {
            ...
            var updated = await UpsertClaimedAutoGeneratedJournalEntryAsync(
                journalEntry,
                workingEntries,
                entry => MatchesInvoicePrePaymentReceivedUpsert(
                    entry,
                    prePaymentAccountId,
                    invoice,
                    paymentLedgerLine,
    ...
    private async Task<AccountingJournalEntryResult> UpsertPrePaymentApplyJournalEntryAsync(Invoice invoice, LedgerLine paymentLedgerLine, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, List<JournalEntry> workingEntries, int prePaymentAccountId, ISet<Guid> retainedEntryIds, Guid currentUser, bool loadCrossPeriodExpectedContext = false, Invoice? paymentSourceInvoice = null)
    {
        try
        {
            ...
            {
                // Actuals are cash: TransactionDate = payment date; AccountingPeriod stays invoice period in builders.
                await UpsertInvoicePaymentEscrowActualJournalEntriesForPaymentAsync(
                    invoice,
                    paymentLedgerLine,
                    paymentLedgerLine.Amount,
    ...
    private async Task UpsertOwnerActualJournalEntryForPaymentAsync(Invoice invoice, LedgerLine paymentLedgerLine, decimal paymentAmount, DateOnly transactionDate, List<JournalEntry> workingEntries, ISet<Guid> retainedEntryIds, Guid currentUser, bool loadCrossPeriodExpectedContext = false, Invoice? paymentSourceInvoice = null)
    {
        ...
        try
        {
            ...
            {
                // Keep this payment's existing OwnAct out of the orphan delete list when rebuild is a no-op.
                var existingOwnerActuals = ClaimAllMatchingEntries(
                    workingEntries,
                    entry => IsInvoiceOwnerActualPaymentJournalEntry(entry, invoice, paymentLedgerLine)
                        && MatchesJournalEntryAccountingPeriod(entry, invoice.AccountingPeriod));
                foreach (var existing in existingOwnerActuals)
                    retainedEntryIds.Add(existing.JournalEntryId);
                return;
            }
    ...
    private async Task<AccountingJournalEntryResult> CreatePrePaymentApplyJournalEntryWithResultAsync(Invoice invoice, LedgerLine paymentLedgerLine, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, Guid currentUser)
    {
        try
        {
            var journalEntry = await CreateJournalEntryFromPrePaymentApplyAsync(invoice, paymentLedgerLine, chartOfAccounts, accountingOffice, currentUser);
            var created = await CreateAutoGeneratedJournalEntryAsync(journalEntry);
            await TryCreateOwnerActualJournalEntryForPaymentAsync(
                invoice,
                paymentLedgerLine,
                paymentLedgerLine.Amount,
                ResolveInvoicePaymentJournalEntryDate(paymentLedgerLine),
                currentUser);
            await TryCreateInvoicePaymentEscrowActualJournalEntriesForPaymentAsync(
                invoice,
                paymentLedgerLine,
                paymentLedgerLine.Amount,
                ResolveInvoicePaymentJournalEntryDate(paymentLedgerLine),
    ...
    public Task<List<JournalEntry>> CreateJournalEntriesFromInvoicePaymentAsync(InvoicePayment invoicePayment, Guid currentUser)
    {
        if (invoicePayment.PaymentApplications.Count == 0)
            return Task.FromResult(new List<JournalEntry>());

        var paymentIds = invoicePayment.PaymentApplications
            .Select(application => application.PaymentLedgerLine.PaymentId)
            .Where(paymentId => paymentId is { } id && id != Guid.Empty)
            .Distinct()
            .ToList();
    ...
    private async Task<List<JournalEntry>> CreateLegacyJournalEntriesFromInvoicePaymentAsync(InvoicePayment invoicePayment, Guid currentUser)
    {
    ...
    private async Task<JournalEntry> CreateJournalEntryFromPrePaymentReceivedAsync(Invoice invoice, LedgerLine paymentLedgerLine, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, Guid currentUser, DateOnly? targetAccountingPeriod = null)
    {
        // AGENT-NOTE: DO NOT TOUCH.
        ...
        var accountingPeriod = targetAccountingPeriod is { } period && period != default
            ? period
            : FirstDayOfMonth(transactionDate);
