...
public partial class AccountingManager
{
    ...
    public async Task<List<JournalEntry>> CreateJournalEntriesFromBillPaymentDocumentAsync(Guid paymentId, Guid organizationId, Guid currentUser)
    {
        ...
        var existingEntries = (await GetJournalEntriesByPaymentIdCachedAsync(payment.OrganizationId, payment.PaymentId)).ToList();
        existingEntries.AddRange(await GetJournalEntriesForSourceAsync(
        ...
        var updated = await UpsertAutoGeneratedJournalEntryAsync(journalEntry, existingEntries, currentUser, organizationId);
        if (updated != null)
        {
            updated.ModifiedBy = currentUser;
            await UpdateJournalEntryWithoutRetainedEarningsRefreshAsync(updated, requireActiveLines: true);
            journalEntries.Add(updated);
            await RemoveOrphanOpenAutoGeneratedJournalEntriesAsync(existingEntries, [updated.JournalEntryId], organizationId, currentUser);
        }
        else if (payment.BillAllocations.Count > 0)
        {
            throw new Exception($"Bill payment journal entry sync failed for {payment.PaymentCode}.");
        }
