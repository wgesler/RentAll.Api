...
public partial class AccountingManager
{
    ...
    private async Task<JournalEntry?> UpsertAutoGeneratedJournalEntryAsync(JournalEntry rebuiltJournalEntry, IReadOnlyList<JournalEntry> existingEntries, Guid currentUser, Guid organizationId)
    {
        ...
        await PruneOpenDuplicateAutoGeneratedJournalEntriesAsync(candidates, organizationId);
    ...
    private Task DeleteJournalEntriesExceptAsync(IEnumerable<JournalEntry> entries, IReadOnlyCollection<Guid> retainJournalEntryIds, Guid organizationId)
        => RemoveOrphanOpenAutoGeneratedJournalEntriesAsync(entries, retainJournalEntryIds, organizationId);
