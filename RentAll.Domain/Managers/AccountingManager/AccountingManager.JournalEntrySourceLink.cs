...
public partial class AccountingManager
{
    ...
    private async Task<JournalEntry?> UpsertAutoGeneratedJournalEntryAsync(JournalEntry rebuiltJournalEntry, IReadOnlyList<JournalEntry> existingEntries, Guid currentUser, Guid organizationId, bool allowReloadFallback = true)
    {
        ...
        if (existingEntry == null && allowReloadFallback)
    ...
    private async Task<JournalEntry?> ResolveExistingJournalEntryBeforeCreateAsync(JournalEntry rebuiltJournalEntry, Guid organizationId, Guid currentUser)
    {
        ...
        var reloaded = await GetDocumentJournalEntriesForSyncAsync(organizationId, rebuiltJournalEntry.OfficeId, (SourceType)rebuiltJournalEntry.SourceTypeId, sourceId);

        if (rebuiltJournalEntry.JournalEntryKindId != default)
            reloaded = reloaded.Where(entry => entry.JournalEntryKindId == rebuiltJournalEntry.JournalEntryKindId).ToList();
    ...
    private async Task<JournalEntry?> UpsertClaimedAutoGeneratedJournalEntryAsync(JournalEntry rebuiltJournalEntry, List<JournalEntry> workingEntries, Func<JournalEntry, bool> predicate, ISet<Guid> retainedEntryIds, Guid currentUser, Guid organizationId, bool allowReloadFallback = true)
    {
        var claimed = ClaimAllMatchingEntries(workingEntries, predicate);
        var updated = await UpsertAutoGeneratedJournalEntryAsync(rebuiltJournalEntry, claimed, currentUser, organizationId, allowReloadFallback);
        if (updated != null)
