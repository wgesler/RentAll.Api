...
public partial class AccountingManager
{
    ...
    private async Task CreateJournalEntryForDepartedReservationAsync(Guid organizationId, ReservationDeparture reservation, CancellationToken cancellationToken, bool logDecisions = false)
    {
        ...
        if (departureFeeCostCode == null && petFeeCostCode == null)
            return;
        ...
        var departureFeeAmount = 0m;
        if (departureFeeCostCode != null)
        {
            var invoicedDepartureFeeTotal = await SumInvoicedLedgerLinesForReservationByCostCodeAsync(organizationId, reservation.OfficeId, reservation.ReservationId, departureFeeCostCode.CostCodeId);
            departureFeeAmount = invoicedDepartureFeeTotal > 0m ? invoicedDepartureFeeTotal : (reservationDetail.DepartureFee > 0m ? reservationDetail.DepartureFee : 0m);
        }

        var petFeeAmount = 0m;
        if (petFeeCostCode != null)
        {
            var invoicedPetFeeTotal = await SumInvoicedLedgerLinesForReservationByCostCodeAsync(organizationId, reservation.OfficeId, reservation.ReservationId, petFeeCostCode.CostCodeId);
            petFeeAmount = invoicedPetFeeTotal > 0m ? invoicedPetFeeTotal : (reservationDetail.PetFee > 0 ? reservationDetail.PetFee : 0m);
        }
        ...
        if (logDecisions)
        {
            ...
            if (departureFeeCostCode != null && departureFeeAmount > 0m)
                feeParts.Add($"departure fee ${departureFeeAmount:0.00}");
            if (petFeeCostCode != null && petFeeAmount > 0m)
                feeParts.Add($"pet fee ${petFeeAmount:0.00}");

            await LogDepartureFeeDecisionAsync(organizationId, reservation.OfficeId, reservation.PropertyId, reservation.PropertyCode, reservation.ReservationCode, reservation.DepartureDate, amount: departureFeeAmount + petFeeAmount, $"Created journal entry — {string.Join(", ", feeParts)}.");
        }
    }
    #endregion
    ...
    private async Task<JournalEntry?> BuildJournalEntryFromDepartedReservationAsync(Guid organizationId, ReservationDeparture reservation, Reservation reservationDetail, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, decimal departureFeeAmount, decimal petFeeAmount, CostCode? departureFeeCostCode, CostCode? petFeeCostCode)
    {
        ...
        int? departureIncomeAccountId = null;

        if (departureFeeCostCode != null && departureFeeAmount > 0m)
        {
            var departureExpenseAccountId = GetDefaultDepartureExpense(chartOfAccounts, reservation.OfficeId, departureFeeCostCode);
            if (departureExpenseAccountId != null)
            {
                departureIncomeAccountId ??= GetDefaultDepartureIncome(chartOfAccounts, reservation.OfficeId, accountingOffice);
                var departureExpenseLine = new JournalEntryLine
                {
                    ChartOfAccountId = departureExpenseAccountId.Value,
                ...
                {
                    ChartOfAccountId = departureIncomeAccountId.Value,
                    CostCodeId = departureFeeCostCode.CostCodeId,
                    Debit = 0m,
        ...
        if (petFeeCostCode != null && petFeeAmount > 0m)
        {
            var petExpenseAccountId = GetDefaultPetExpense(chartOfAccounts, reservation.OfficeId, petFeeCostCode);
            if (petExpenseAccountId != null)
            {
                departureIncomeAccountId ??= GetDefaultDepartureIncome(chartOfAccounts, reservation.OfficeId, accountingOffice);
                var petExpenseLine = new JournalEntryLine
                {
                    ChartOfAccountId = petExpenseAccountId.Value,
