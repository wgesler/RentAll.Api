...
public partial class AccountingManager
{
    ...
    private async Task<JournalEntry> UpdateAutoGeneratedJournalEntryAsync(JournalEntry journalEntry, bool postImmediately = false)
    {
        ...
        if (existing.PostingStatusId == PostingStatus.HardClosed)
            throw new Exception($"Journal entry {existing.JournalEntryCode} is hard-closed and cannot be changed.");

        return await UpdateJournalEntryAsync(journalEntry, requireActiveLines: HasActiveJournalEntryLines(journalEntry), postImmediately: postImmediately);
    }
    ...
    private async Task DeleteJournalEntriesForPaymentAsync(Payment payment)
    {
        ...
        var ownerPaymentEntries = await GetJournalEntriesForSourceAsync(
            payment.OrganizationId,
            payment.OfficeId,
            SourceType.OwnerDistribution,
            payment.PaymentId);

        foreach (var entry in ownerPaymentEntries)
            await DeleteOpenJournalEntryAsync(entry.JournalEntryId, payment.OrganizationId);
    }
