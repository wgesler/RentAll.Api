...
public partial class AccountingManager
{
    ...
    private async Task BuildJournalEntiesForDepartedReservationAsync(Guid organizationId, ReservationList reservation, CancellationToken cancellationToken)
    {
        ...
        {
            await LogPeriodicAccountingDecisionAsync(
                organizationId,
                reservation.OfficeId,
                reservation.PropertyId,
                null,
                success: false,
                $"Departures skipped for reservation {reservation.ReservationCode}: journal entry already exists.");
            return;
        }
        ...
        if (departureFeeAmount == 0m && petFeeAmount == 0m)
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                0m,
                success: false,
                $"Departures skipped for reservation {reservation.ReservationCode}: DepartureFee and PetFee are both zero.");
            return;
        }
        ...
        await LogPeriodicAccountingDecisionAsync(
            ...
            reservation.OfficeId,
            reservation.PropertyId,
            departureFeeAmount + petFeeAmount,
            success: created != null,
            created == null
                ? $"Departures not created for reservation {reservation.ReservationCode}: accounting feature is disabled."
                : $"Departures created for reservation {reservation.ReservationCode}: JE {created.JournalEntryCode}, DepartureFee={departureFeeAmount:0.00}, PetFee={petFeeAmount:0.00}.");
    }
    ...
    private async Task BuildJournalEntriesForLinensAndTowelsAsync(Guid organizationId, PropertyAgreement agreement, bool isMonthly, DateOnly processingDate, CancellationToken cancellationToken)
    {
        ...
        if (existingEntries.Any())
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                agreement.LinenAndTowelFee,
                success: false,
                $"LinensAndTowels skipped for property {agreement.PropertyId}: journal entry already exists for {processingDate:yyyy-MM-dd}.");
            return;
        }
        ...
        if (property.PropertyLeaseType != PropertyLeaseType.PropertyManagement)
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                agreement.LinenAndTowelFee,
                success: false,
                $"LinensAndTowels skipped for property {agreement.PropertyId}: lease type is {property.PropertyLeaseType}, expected PropertyManagement.");
            return;
        }
        ...
        if (isMonthly)
            await BuildMonthlyJournalEntriesForLinensAndTowelsAsync(organizationId, agreement, availableFrom, availableUntil, processingDate, cancellationToken);
        else
            await BuildAnnualJournalEntriesForLinensAndTowelsAsync(organizationId, agreement, availableFrom, availableUntil, processingDate, cancellationToken);
    }

    private async Task BuildMonthlyJournalEntriesForLinensAndTowelsAsync(Guid organizationId, PropertyAgreement agreement, DateOnly? availableFrom, DateOnly? availableUntil, DateOnly processingDate, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        if (!wasRentedPreviousMonth)
        {
            ...
            fallbackUsed = wasRentedPreviousMonth;
        }
        ...
        if (!wasRentedPreviousMonth)
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                agreement.LinenAndTowelFee,
                success: false,
                $"Monthly LinensAndTowels skipped for property {agreement.PropertyId}: no occupancy overlap found in previous month for run date {processingDate:yyyy-MM-dd}.");
            return;
        }
        ...
        if (fallbackUsed)
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                agreement.LinenAndTowelFee,
                success: true,
                $"Monthly LinensAndTowels fallback eligibility used for property {agreement.PropertyId}: SQL previous-month check was false but reservation overlap check was true.");
        }

        await BuildLinenAndTowelEntriesAsync(organizationId, agreement, availableFrom, availableUntil, isMonthly: true, processingDate, cancellationToken);
    }

    private async Task BuildAnnualJournalEntriesForLinensAndTowelsAsync(Guid organizationId, PropertyAgreement agreement, DateOnly? availableFrom, DateOnly? availableUntil, DateOnly processingDate, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        if (!isAvailableFromCurrentMonthAndYear && !isAvailableUntilCurrentMonthAndYear && processingDate.Month != 1)
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                agreement.LinenAndTowelFee,
                success: false,
                $"Annual LinensAndTowels skipped for property {agreement.PropertyId}: run date {processingDate:yyyy-MM-dd} is not onboarding month, offboarding month, or January.");
            return;
        }

        await BuildLinenAndTowelEntriesAsync(organizationId, agreement, availableFrom, availableUntil, isMonthly: false, processingDate, cancellationToken);
    }

    private async Task BuildLinenAndTowelEntriesAsync(Guid organizationId, PropertyAgreement agreement, DateOnly? availableFrom, DateOnly? availableUntil, bool isMonthly, DateOnly processingDate, CancellationToken cancellationToken)
    {
        // AGENT-NOTE: DO NOT TOUCH.
        ...
        if (feeAmount <= 0m)
        {
            await LogPeriodicAccountingDecisionAsync(
                ...
                feeAmount,
                success: false,
                $"{(isMonthly ? "Monthly" : "Annual")} LinensAndTowels skipped for property {agreement.PropertyId}: calculated fee amount is {feeAmount:0.00} for run date {processingDate:yyyy-MM-dd}.");
            return;
        }
        ...
        await LogPeriodicAccountingDecisionAsync(
            ...
            success: created != null,
            created == null
                ? $"{cadenceLabel} LinensAndTowels not created for property {agreement.PropertyId}: accounting feature is disabled."
                : $"{cadenceLabel} LinensAndTowels created for property {agreement.PropertyId}: JE {created.JournalEntryCode}, amount={feeAmount:0.00}, reversedDirection={reverseEntryDirection}.");
    }
    ...
    private async Task LogPeriodicAccountingDecisionAsync(Guid organizationId, int officeId, Guid propertyId, decimal? amount, bool success, string message)
    {
        await LogAccountingLogAsync(new AccountingLog
        {
            ...
            Message = $"{(success ? "Success" : "Failure")}: {message}"
        });
    }
    #endregion
}
