...
public partial class AccountingManager : IAccountingManager
{
    ...
    /// </summary>
    private static List<ChartOfAccount> GetRentalIncomeAccounts(List<ChartOfAccount> chartOfAccounts, int officeId, Office? office, IReadOnlyDictionary<int, CostCode> costCodeById, AccountingOffice? accountingOffice)
    {
        var anchorAccount = ResolveRentalIncomeAnchorAccountFromOffice(chartOfAccounts, officeId, office, costCodeById, accountingOffice);
        var rootAccount = ResolveRentalIncomeRootAccount(anchorAccount, chartOfAccounts, officeId);
        return CollectRentalIncomeAccountTree(chartOfAccounts, officeId, rootAccount);
    }

    private static ChartOfAccount ResolveRentalIncomeAnchorAccountFromOffice(List<ChartOfAccount> chartOfAccounts, int officeId, Office? office, IReadOnlyDictionary<int, CostCode> costCodeById, AccountingOffice? accountingOffice)
    {
        CostCode? rentCostCode = null;
        if (office?.FurnishedRentChargeCcId is > 0)
            costCodeById.TryGetValue(office.FurnishedRentChargeCcId.Value, out rentCostCode);
        if (rentCostCode == null && office?.UnfurnishedRentChargeCcId is > 0)
            costCodeById.TryGetValue(office.UnfurnishedRentChargeCcId.Value, out rentCostCode);

        var defaultTenantIncAccountId = ResolveDefaultTenantIncAccountId(chartOfAccounts, officeId, accountingOffice);
        var accountId = GetChartOfAccountIdByCostCode(chartOfAccounts, officeId, rentCostCode, defaultTenantIncAccountId);
        var account = chartOfAccounts.FirstOrDefault(a => a.OfficeId == officeId && a.AccountId == accountId);
        if (account == null)
            throw new Exception($"No rental income chart of account is configured for office {officeId}");

        return account;
    }

    private static int ResolveDefaultTenantIncAccountId(List<ChartOfAccount> chartOfAccounts, int officeId, AccountingOffice? accountingOffice)
    {
        if (accountingOffice?.DefaultTenantIncAccountId is > 0)
    ...
    private static ChartOfAccount ResolveRentalIncomeRootAccount(ChartOfAccount account, IReadOnlyList<ChartOfAccount> chartOfAccounts, int officeId)
    {
        var current = account;
        var visitedAccountIds = new HashSet<int>();
        while (current.IsSubaccount && current.SubAccountId is > 0 && visitedAccountIds.Add(current.AccountId))
        {
            var parent = chartOfAccounts.FirstOrDefault(a =>
                a.OfficeId == officeId
    ...
    private static List<ChartOfAccount> CollectRentalIncomeAccountTree(IReadOnlyList<ChartOfAccount> chartOfAccounts, int officeId, ChartOfAccount rootAccount)
    {
        var rentalAccounts = new List<ChartOfAccount> { rootAccount };
        var childAccountIds = new Queue<int>();
        childAccountIds.Enqueue(rootAccount.AccountId);

        while (childAccountIds.Count > 0)
        {
            var parentAccountId = childAccountIds.Dequeue();
            foreach (var childAccount in chartOfAccounts.Where(a =>
                a.OfficeId == officeId
                && a.IsSubaccount
                && a.SubAccountId == parentAccountId))
            {
                rentalAccounts.Add(childAccount);
                childAccountIds.Enqueue(childAccount.AccountId);
            }
        }

        return rentalAccounts
            .OrderBy(a => NormalizeAccountCode(a.AccountNo))
