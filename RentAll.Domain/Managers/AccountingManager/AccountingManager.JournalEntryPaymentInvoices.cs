...
public partial class AccountingManager
{
    ...
    private async Task CreateJournalEntriesFromPaymentApplicationsAsync(Payment payment, IReadOnlyList<PaymentApplicationContext> applications, Guid currentUser, PaymentJournalEntryCreateResult result)
    {
        ...
        var mainCashWorkingEntries = existingPaymentDocumentEntries
            .Where(IsInvoicePaymentCashJournalEntry)
            .ToList();
        foreach (var stampedEntry in await GetJournalEntriesByPaymentIdCachedAsync(payment.OrganizationId, payment.PaymentId))
        {
            if (!IsInvoicePaymentCashJournalEntry(stampedEntry))
                continue;
            if (mainCashWorkingEntries.Any(entry => entry.JournalEntryId == stampedEntry.JournalEntryId))
                continue;
            mainCashWorkingEntries.Add(stampedEntry);
        }
        ...
        foreach (var application in applications)
        {
            ...
            foreach (var existingEntry in existingPaymentEntries.Where(IsInvoicePaymentCashJournalEntry))
            {
                if (mainCashWorkingEntries.Any(entry => entry.JournalEntryId == existingEntry.JournalEntryId))
                    continue;
                mainCashWorkingEntries.Add(existingEntry);
            }
        }

        result.Note($"Main cash Payment JE candidates: {mainCashWorkingEntries.Count}");

        var retainedEntryIds = new HashSet<Guid>();
        var consolidatedPaymentEntry = await CreateConsolidatedInvoicePaymentJournalEntryAsync(
            payment,
            applications,
            currentUser);
        var upsertedPaymentEntry = await UpsertClaimedAutoGeneratedJournalEntryAsync(
            consolidatedPaymentEntry,
            mainCashWorkingEntries,
            IsInvoicePaymentCashJournalEntry,
            retainedEntryIds,
            currentUser,
            payment.OrganizationId);

        if (upsertedPaymentEntry == null)
        {
            result.Bail("Consolidated Payment JE upsert returned null.");
            return;
        }

        result.Note(
            $"Upserted consolidated {upsertedPaymentEntry.JournalEntryCode} "
            + $"kind={(int)upsertedPaymentEntry.JournalEntryKindId} PaymentId={upsertedPaymentEntry.PaymentId} "
            + $"UF={payment.Amount:0.00}");
        result.JournalEntries.Add(upsertedPaymentEntry);
        await DeleteClaimedOrphanJournalEntriesAsync(mainCashWorkingEntries, payment.OrganizationId, payment.PaymentId);
        ...
        foreach (var application in applications)
        {
            ...
            if (paymentResult.JournalEntry != null
                && !result.JournalEntries.Any(entry => entry.JournalEntryId == paymentResult.JournalEntry.JournalEntryId))
            {
    ...
    private async Task<JournalEntry> CreateConsolidatedInvoicePaymentJournalEntryAsync(Payment payment, IReadOnlyList<PaymentApplicationContext> applications, Guid currentUser)
    {
        ...
        if (applications.Count == 0)
            throw new Exception("At least one invoice allocation is required.");
        ...
        if (!costCodeById.TryGetValue(payment.CostCodeId, out var paymentCostCode) || !IsPaymentLedgerLine(paymentCostCode))
            throw new Exception("Payment must use a payment cost code");
        ...
        foreach (var application in applications)
        {
            ...
            var lineContext = await ResolveInvoiceJournalEntryLineContextAsync(
                application.Invoice,
                application.PaymentLedgerLine);
        ...
        if (journalEntryLines.Count == 0)
            throw new Exception("Payment has no non-zero invoice allocations.");
        ...
        var undepositedFundsLine = new JournalEntryLine
        {
            ...
            Debit = ufAmount > 0 ? ufAmount : 0,
            Credit = ufAmount < 0 ? Math.Abs(ufAmount) : 0,
            Memo = ufMemo,
            CreatedBy = currentUser
        };
        if (ufLineContext is { } resolvedUfContext)
            ApplyJournalEntryLineContext(undepositedFundsLine, resolvedUfContext);

        journalEntryLines.Insert(0, undepositedFundsLine);
