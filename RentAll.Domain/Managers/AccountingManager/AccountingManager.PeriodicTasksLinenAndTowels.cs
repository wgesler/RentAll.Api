...
public partial class AccountingManager
{
    ...
    private async Task CreateJournalEntryForLinenAndTowelAsync(Guid organizationId, PropertyAgreement agreement, bool isMonthly, DateOnly processingDate, CancellationToken cancellationToken)
    {
        ...
        if (isMonthly)
            await CreateMonthlyJournalEntryForLinenAndTowelAsync(organizationId, agreement, property, propertyCode, availableFrom, availableUntil, processingDate, cancellationToken);
        else
            await CreateAnnualJournalEntryForLinenAndTowelAsync(organizationId, agreement, property, propertyCode, availableFrom, availableUntil, processingDate, cancellationToken);
    }

    private async Task CreateMonthlyJournalEntryForLinenAndTowelAsync(Guid organizationId, PropertyAgreement agreement, Property property, string propertyCode, DateOnly? availableFrom, DateOnly? availableUntil, DateOnly processingDate, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        await CreateJournalEntryForLinenAndTowelAgreementAsync(organizationId, agreement, property, propertyCode, availableFrom, availableUntil, isMonthly: true, processingDate, cancellationToken);
    }

    private async Task CreateAnnualJournalEntryForLinenAndTowelAsync(Guid organizationId, PropertyAgreement agreement, Property property, string propertyCode, DateOnly? availableFrom, DateOnly? availableUntil, DateOnly processingDate, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        await CreateJournalEntryForLinenAndTowelAgreementAsync(organizationId, agreement, property, propertyCode, availableFrom, availableUntil, isMonthly: false, processingDate, cancellationToken);
    }

    private async Task CreateJournalEntryForLinenAndTowelAgreementAsync(Guid organizationId, PropertyAgreement agreement, Property property, string propertyCode, DateOnly? availableFrom, DateOnly? availableUntil, bool isMonthly, DateOnly processingDate, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        var journalEntry = await BuildJournalEntryFromLinenAndTowelAsync(organizationId, agreement, property, propertyCode, chartOfAccounts, accountingOffice, feeAmount, reverseEntryDirection, isMonthly, processingDate);
        await CreateAutoGeneratedJournalEntryAsync(journalEntry);
    }
    #endregion

    #region Journal Entries
    private async Task<JournalEntry> BuildJournalEntryFromLinenAndTowelAsync(Guid organizationId, PropertyAgreement agreement, Property property, string propertyCode, List<ChartOfAccount> chartOfAccounts, AccountingOffice? accountingOffice, decimal feeAmount, bool reverseEntryDirection, bool isMonthly, DateOnly processingDate)
    {
        ...
        var lineContext = await ResolveOwnerJournalEntryLineContextForPropertyAsync(organizationId, property);
        ...
        var incomeLine = new JournalEntryLine
        {
