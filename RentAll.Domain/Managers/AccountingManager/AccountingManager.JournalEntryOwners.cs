...
public partial class AccountingManager
{
        ...
        Guid currentUser)
    {
        ...
        var ownerShareExisting = existingInvoiceEntries.FirstOrDefault(IsInvoiceOwnerShareJournalEntry);
    ...
    private async Task<AccountingJournalEntryResult> CreateJournalEntryFromWorkOrderWithResultAsync(WorkOrder workOrder, Guid currentUser)
    {
        ...
        try
        {
            ...
            if (created != null)
                await PersistWorkOrderJournalEntryIdIfChangedAsync(workOrder, created.JournalEntryId, currentUser);
    ...
    private async Task<JournalEntry?> BuildJournalEntryFromPaymentForOwnerActualAsync(Invoice invoice, LedgerLine paymentLedgerLine, decimal paymentAmount, DateOnly transactionDate, Guid currentUser)
    {
        var propertyId = await ResolveInvoicePropertyIdAsync(invoice);
        if (propertyId == null || propertyId == Guid.Empty)
            return null;
        ...
        var actualAmount = await CalculateOwnerActualAmountForInvoicePaymentAsync(invoice, paymentAmount);
    ...
    private async Task<decimal?> CalculateOwnerActualAmountForInvoicePaymentAsync(Invoice invoice, decimal paymentAmount)
    {
        ...
        return await CalculateOwnerShareAmountForInvoiceAsync(ownerShareInvoice, rentPaymentAmount);
    }
