...
public partial class AccountingManager
{
    ...
    private async Task<JournalEntry?> UpsertJournalEntryFromInvoiceForOwnerShareAsync(Invoice invoice, IReadOnlyList<JournalEntry> existingInvoiceEntries, Guid currentUser)
    {
    ...
    private async Task<decimal?> ResolveOwnerShareAmountAsync(Invoice invoice, decimal? paymentAmount = null)
    {
        if (!paymentAmount.HasValue)
            return await ResolveOwnerExpectedShareAmountAsync(invoice);

        var ownerShareInvoice = await ResolveInvoiceForOwnerShareAsync(invoice);
        var chargeBases = await GetInvoiceChargeBasesAsync(ownerShareInvoice);
        if (chargeBases.RentalIncome == 0)
            return null;

        if (paymentAmount.Value == 0)
            return null;

        if (chargeBases.ChargeTotal == 0)
            return null;

        var rentPaymentAmount = paymentAmount.Value <= chargeBases.NonRentTotal
            ? 0m
            : Math.Min(paymentAmount.Value - chargeBases.NonRentTotal, chargeBases.RentalIncome);
    ...
    private async Task<decimal?> ResolveOwnerExpectedShareAmountAsync(Invoice invoice)
    {
        var chargeBases = await GetInvoiceChargeBasesAsync(invoice);
        if (chargeBases.RentalIncome == 0)
            return null;

        var ownerAmount = await CalculateOwnerShareAmountForInvoiceAsync(invoice, chargeBases.RentalIncome);
        if (!ownerAmount.HasValue || ownerAmount.Value == 0)
            return null;
    ...
    private async Task<decimal?> ResolveOwnerActualAmountForSliceAsync(Invoice sliceInvoice, decimal slicePaymentAmount, IReadOnlyList<JournalEntry>? workingEntries)
    {
        ...
        var sliceChargeBases = await GetInvoiceChargeBasesAsync(ownerShareInvoice);
        if (sliceChargeBases.ChargeTotal > 0 && slicePaymentAmount >= sliceChargeBases.ChargeTotal)
            return ownerExpectedAmount;

        if (sliceChargeBases.RentalIncome <= 0)
            return ownerExpectedAmount;

        if (slicePaymentAmount >= sliceChargeBases.RentalIncome)
            return ownerExpectedAmount;

        return Math.Min(
            ownerExpectedAmount.Value,
            Math.Round(ownerExpectedAmount.Value * slicePaymentAmount / sliceChargeBases.RentalIncome, 2, MidpointRounding.AwayFromZero));
    }
    ...
    private async Task<decimal?> CalculateOwnerShareAmountForInvoiceAsync(Invoice invoice, decimal amount)
    {
        ...
        switch (propertyAgreement.ManagementFeeType)
        {
            case ManagementFeeType.FlatRate:
                if (!TryGetInvoiceRentalLedgerLine(invoice, out var flatRateRentalLine))
                    return null;
                ownerAmount = await GetProratedOwnerFlatAmountAsync(invoice, propertyAgreement.FlatRateAmount, flatRateRentalLine);
                break;
            ...
            case ManagementFeeType.Minimum:
                {
                    if (!TryGetInvoiceRentalLedgerLine(invoice, out var minimumRentalLine))
                        return null;
                    ownerAmount = amount * propertyAgreement.RevenueSplitOwner / 100m;
                    var proratedMinimum = await GetProratedOwnerFlatAmountAsync(invoice, propertyAgreement.FlatRateAmount, minimumRentalLine);
