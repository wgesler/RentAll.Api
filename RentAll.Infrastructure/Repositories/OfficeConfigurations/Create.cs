...
{
	public async Task<OfficeConfiguration> CreateAsync(OfficeConfiguration officeConfiguration)
	{
		await using var db = new SqlConnection(_dbConnectionString);
		var res = await db.DapperProcQueryAsync<OfficeConfigurationEntity>("dbo.OfficeConfiguration_Add", new
		{
		...
		if (res == null || !res.Any())
			throw new Exception("OfficeConfiguration not created");
